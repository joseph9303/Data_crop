...
def save_hbox_to_xml(file_name, save_path, im_width, im_height, objects_axis, label_name):
    im_depth = 0
    object_num = len(objects_axis)
    doc = Document()
        ...
        for start_h in range(0, shape[0], 600):
            for start_w in range(0, shape[1], 600):
                ...
                if len(idx) > 0:
                    ...
                    xml_h = os.path.join(save_dir, 'xml', "%s_%04d_%04d.xml" % (file_idx, top_left_row, top_left_col))
                    file_name = ("%s_%04d_%04d.png" % (file_idx, top_left_row, top_left_col))
                    #save_rbox_to_xml(xml_r, subImage.shape[1], subImage.shape[0], box[idx, :], class_list)
                    save_hbox_to_xml(file_name, xml_h, subImage.shape[1], subImage.shape[0], box[idx, :], class_list)
                    # print ('save xml : ', xml)
...
raw_data = '/home/joseph/joseph_data/Dota v1.5/validation/val-20190425T083021Z-001/val/'
raw_images_dir = os.path.join('/home/joseph/joseph_data/Dota v1.5/train/images')
raw_label_dir = os.path.join('/home/joseph/joseph_data/Dota v1.5/train/train/labelTxt-v1.0')

save_dir = '/home/joseph/joseph_data/dota_coco/train_1'
